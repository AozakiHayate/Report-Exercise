...
to setup
  clear-all
  setup-patches
  setup-isolate　　　;;　オブザーバー側隔離区の設定を読んでいる
  setup-vaccine
  setup-constants
  setup-turtles
...
to setup-patches
  ...
  ask patches [set pcolor 131] ;;背景の色を131色に設定する

end

to setup-isolate

  ask n-of isolate patches[    　　　　　;;　隔離区の数はランダムでインタフェース部の「isolate」関数から決める
    ask neighbors [set pcolor 27]      ;;　隔離区の色を設定する
    set pcolor 27
  ]
end
...
to setup-vaccine
  ...
  ask n-of vaccine_center patches[    　　　;;　隔離区の数はランダムでインタフェース部の「isolate」関数から決める
    ask neighbors4 [set pcolor white]      ;;　隔離区の色を設定する
    set pcolor white
  ]
end
...
to get-older ;; turtle procedure
  ...
  if pcolor = white  ;;　ワクチン接種センター内の人はワクチンワクチンを接種することにより免疫を得る
  [become-immune]
...
to move ;; turtle procedure
  ...
  if [pcolor] of patch-ahead 0 != 27　;;　隔離区以外の人は自由に動ける
  [fd 1 ]
  if pcolor = 27 　　　　　　　　　　　;;　隔離区以内の人は隔離されたからゆっくり動ける
  [fd 0.05]

  if pcolor = white 　　　　　　　　　　;;　ワクチン接種センター内の人はゆっくり動ける
  [fd 0.1]
