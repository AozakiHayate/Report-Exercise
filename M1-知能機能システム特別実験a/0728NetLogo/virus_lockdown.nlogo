...
;;---------------------------------------------------------------------------------------------------------
to setup-rectricted                          ;; restricted zone
  ask n-of number-restrict patches[          ;; number-restricted is the density of restricted zone, which is defined by the button
    ask neighbors [set pcolor yellow]        ;; the restricted zone is colored in yellow
    set pcolor yellow
  ]
end
;;----------------------------------------------------------------------------------------------------------
...
;; Turtles move about at random and avoid getting into restricted
;;-----------------------------------------------------------------------------------------------
to move ;; turtle procedure
  rt random 100
  lt random 100

  ;; get away if the turtles get into the restricted zone by accident
  if pcolor = yellow
  [fd 1]

  ;; move only when there is no restricted zone ahead to avoid getting into restricted zone
